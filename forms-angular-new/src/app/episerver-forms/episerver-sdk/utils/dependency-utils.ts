...
export function getValueOfDependeeElement(condition: Condition, formSubmissions: FormSubmission[], elementDependencies: ElementDependencies[]): unknown {
  let dependeeFieldValue = formSubmissions.filter(submission => equals(submission.elementKey, condition.field))[0]?.value;
