...
export const FIELDS_TO_IGNORE = ['FormStepBlock', 'SubmitButtonElementBlock', 'PredefinedHiddenElementBlock', 'ParagraphTextElementBlock'];
...
export function getConfirmationData(data: FormSubmission[], form: FormContainer, currentStepIndex: number, inactiveElements: string[], fieldsToIgnore?: string[]): string {
  ...
  data.forEach(elementData => {
    ...
    if (formElement && ignoreFields.indexOf(formElement.contentType) === -1 && inactiveElements.indexOf(formElement.key) === -1 && !isNullOrEmpty(String(value ?? ''))) {
