...
export class FormConfirmationService {
  ...
  buildSummary(form: FormSchema, formGroup: FormGroup, currentStepIndex: number): string {
    const steps = this.formSchemaFormService.buildForm(form).steps;
    ...
    return step.elements
      .filter((field: FormField) => !this.isNonDataField(field))
      .map((field: FormField) => this.formatFieldValue(field, formGroup.get(field.key)?.value))
      .filter(Boolean)
      .join('\n');
  }
