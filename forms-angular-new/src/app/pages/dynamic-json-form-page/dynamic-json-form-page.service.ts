...
const DEBUG_DYNAMIC_JSON_FORM: DynamicEpiServerForm = {
  formId: 31777,
...
export class DynamicJsonFormPageService {
  readonly endpoint = 'http://localhost:8000/test-episerver-form';
  readonly debug = true;

  constructor(private readonly httpClient: HttpClient) {}

  loadForm(): Observable<DynamicEpiServerForm> {
    if (this.debug) {
      return of(this.cloneForm(DEBUG_DYNAMIC_JSON_FORM));
    }

    return this.httpClient.get<DynamicEpiServerForm>(this.endpoint);
  }

  savePartial(source: DynamicEpiServerForm, model: FormSubmitModel): Observable<FormSubmissionResult> {
    return this.httpClient.post<FormSubmissionResult>(this.endpoint, this.toFormData(model), {
      headers: this.buildHeaders(source)
    });
  }

  submitFinal(source: DynamicEpiServerForm, model: FormSubmitModel): Observable<FormSubmissionResult> {
    return this.httpClient.put<FormSubmissionResult>(this.endpoint, this.toFormData(model), {
      headers: this.buildHeaders(source)
    });
  }

  private buildHeaders(source: DynamicEpiServerForm): HttpHeaders | undefined {
  ...
  private cloneForm(source: DynamicEpiServerForm): DynamicEpiServerForm {
    return JSON.parse(JSON.stringify(source)) as DynamicEpiServerForm;
  }
}
